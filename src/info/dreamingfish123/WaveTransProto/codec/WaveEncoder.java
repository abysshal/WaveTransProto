...
public class WaveEncoder {
	...
	public static byte[] encode(byte[] data) {
		byte[] ret = new byte[data.length * Constant.BIT_PER_BYTE
				* Constant.POINT_PER_SAMPLE + Constant.WAVE_HEAD_LEN];
		// Get wave header
		...
		Util.int2byte(data.length * Constant.BIT_PER_BYTE
				* Constant.POINT_PER_SAMPLE, ret, Constant.WAVE_DATA_LEN_OFFSET);
		// set file length field in header
		Util.int2byte(data.length * Constant.BIT_PER_BYTE
				* Constant.POINT_PER_SAMPLE + Constant.WAVE_HEAD_LEN - 8, ret,
	...
	private static void convertBit(byte[] ret, int offset, int bit) {
		int index = offset;
		for (int i = 0; i < Constant.POINT_PER_SAMPLE / 2; i++) {
